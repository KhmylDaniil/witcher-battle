...
/**
 * Интерактивная карта боя (гексы + фишки участников) — общий кусок для отдельного окна
 * (BattleMapWindowPage) и встроенной панели на странице боя (BattleDetailsPage, когда к бою
 * подключена карта). Мастер расставляет участников свободно (place, доступно всегда); в свой ход
 * контроллер активного участника вместо этого видит подсветку гексов, на которые можно дойти по
 * правилам движения (move, тратит очки движения) — см. Battle.MoveParticipant на бэке. Требует
 * flex-контейнер с заданной высотой снаружи (сам занимает flex-1).
 */
export function BattleMapBoard({ gameId, view }: { gameId: number; view: BattleMapView }) {
  ...
  // 'movement-range': область подсветки остатка движения обязана обновляться вместе с картой после
  // любого изменения расстановки/позиции, не только после самого move.
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-neutral-100 dark:border-neutral-800 dark:bg-neutral-950">
      <TerrainPatternDefs />
      {error && (
