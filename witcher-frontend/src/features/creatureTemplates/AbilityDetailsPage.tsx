...
export function AbilityDetailsPage() {
  const { gameId, creatureTemplateId, abilityId } = useParams<{ gameId: string; creatureTemplateId: string; abilityId: string }>()
  const creatureTemplateIdNum = Number(creatureTemplateId)
  const abilityIdNum = Number(abilityId)
  const navigate = useNavigate()
  const queryClient = useQueryClient()

  const creatureTemplate = useQuery({
    queryKey: ['creature-templates', creatureTemplateIdNum],
    queryFn: () => creatureTemplatesApi.get(creatureTemplateIdNum),
  })
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['creature-templates', creatureTemplateIdNum] })
  ...
  const updateAbility = useMutation({
    mutationFn: () => creatureTemplatesApi.updateAbility(creatureTemplateIdNum, abilityIdNum, abilityValues!),
    onSuccess: async () => {
      await invalidate()
  ...
  const removeAbility = useMutation({
    mutationFn: () => creatureTemplatesApi.removeAbility(creatureTemplateIdNum, abilityIdNum),
    onSuccess: async () => {
      await invalidate()
      navigate(`/games/${gameId}/creature-templates/${creatureTemplateIdNum}`)
    },
  })
  ...
  const updateCondition = useMutation({
    mutationFn: (conditionId: number) =>
      creatureTemplatesApi.updateCondition(creatureTemplateIdNum, abilityIdNum, conditionId, editCondition.condition, editCondition.applyChance),
    onSuccess: () => {
      invalidate()
  ...
  const removeCondition = useMutation({
    mutationFn: (conditionId: number) => creatureTemplatesApi.removeCondition(creatureTemplateIdNum, abilityIdNum, conditionId),
  ...
  const addCondition = useMutation({
    mutationFn: () => creatureTemplatesApi.addCondition(creatureTemplateIdNum, abilityIdNum, newCondition.condition, newCondition.applyChance),
    onSuccess: () => {
      invalidate()
  ...
  const removeDefensiveSkill = useMutation({
    mutationFn: (defensiveSkillId: number) => creatureTemplatesApi.removeDefensiveSkill(creatureTemplateIdNum, abilityIdNum, defensiveSkillId),
    onSuccess: invalidate,
  })
  const [newDefensiveSkill, setNewDefensiveSkill] = useState<Skill>('Dodge')
  const addDefensiveSkill = useMutation({
    mutationFn: () => creatureTemplatesApi.addDefensiveSkill(creatureTemplateIdNum, abilityIdNum, newDefensiveSkill),
    onSuccess: () => {
      invalidate()
  ...
  const ability = ct?.abilities.find((a) => a.id === abilityIdNum)
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          <>
            <Link to={`/games/${gameId}/creature-templates/${creatureTemplateIdNum}`}>
              <Button variant="secondary">К шаблону существа</Button>
            </Link>
            {!editingAbility && (
              <Button
                variant="secondary"
