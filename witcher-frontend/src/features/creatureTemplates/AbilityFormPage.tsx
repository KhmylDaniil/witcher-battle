...
export function AbilityFormPage() {
  ...
  const save = useMutation({
    mutationFn: (values: AbilityFormValues) => creatureTemplatesApi.addAbility(creatureTemplateIdNum, values),
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ['creature-templates', creatureTemplateIdNum] })
      const created = result.abilities[result.abilities.length - 1]
      navigate(`/games/${gameId}/creature-templates/${creatureTemplateIdNum}/abilities/${created.id}`)
    },
  })
